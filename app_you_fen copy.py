...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        # 暂停\结束 按钮
        self.pause_button = customtkinter.CTkButton(self, text="Pause",width=10, command=self.pause_download)
        self.stop_button = customtkinter.CTkButton(self, text="Stop", width=10, command=self.stop_download)
    ...
    def merge_pdf(self):
        self.stop_event.clear()
        self.current_thread = threading.Thread(target=self.merge_pdf_in_background, daemon=True)
        self.current_thread.start()
    ...
    def pause_download(self):
        ...
        self.pause_button.configure(text="Resume", command=self.resume_download)

    def resume_download(self):
        self.crawler.pause_requested.clear()
        self.text_1.insert("end", '\n- Resume download -\n\n')
        # Change the text and the command of the Resume button back to Pause
        self.pause_button.configure(text="Pause", command=self.pause_download)
